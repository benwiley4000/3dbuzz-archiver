...
const { UINT8_VIEW_SIZE, OUTPUT_ZIP_DIRECTORY } = require('../constants');
...
let shouldSanitizeFilenames = false;
try {
  const testFilename = path.join(
    OUTPUT_ZIP_DIRECTORY,
    'Filename: might not be valid?'
  );
  fs.writeFileSync(testFilename, '');
  fs.unlinkSync(testFilename);
} catch(err) {
  shouldSanitizeFilenames = true;
}
function sanitizeFilename(name) {
  if (shouldSanitizeFilenames) {
    // skipping illegal exFAT characters EXCEPT for
    // forward and back slash because these represent
    // directories
